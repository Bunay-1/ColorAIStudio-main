...
async def vision_analyze(request: Request, file: UploadFile = File(...), generate_map: bool = False, current_user: dict = Depends(get_current_user)):
    ...
    response = await vision_service.analyze_vision(file, generate_map, request.app.state.icap, request.client.host if request.client else None)
...
async def vision_fusion(request: Request, files: List[UploadFile] = File(...), current_user: dict = Depends(get_current_user)):
    """Ендпоинт за Fusion на изгледи."""
    check_user_rate_limit(current_user.get("username", "anonymous"), current_user.get("role", "OPERATOR"), "heavy")
    
    response = await vision_service.fuse_vision_views(files, request.app.state.icap)
...
async def vision_micro_analyze(request: Request, file: UploadFile = File(...), current_user: dict = Depends(get_current_user)):
    """Ендпоинт за микро-дефекти."""
    check_user_rate_limit(current_user.get("username", "anonymous"), current_user.get("role", "OPERATOR"), "heavy")
    
    response = await vision_service.analyze_micro_vision(file, request.app.state.icap)
